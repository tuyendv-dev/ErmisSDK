...
internal class ChannelMutableState(
    ...
    val messageList: StateFlow<List<Message>> =
        combineStates(_messages!!, latestUsers) { messageMap, userMap -> messageMap.values.updateUsers(userMap, _channelData?.value?.name) }
    ...
    fun addQuotedMessage(quotedMessageId: String, quotingMessageId: String) {
        _quotedMessagesMap?.apply {
            val quotesMap = value
            if ((quotesMap[quotedMessageId] ?: listOf()).contains(quotingMessageId).not()) {
                quotesMap[quotedMessageId] =
                    quotesMap[quotedMessageId]?.plus(quotingMessageId) ?: listOf(quotingMessageId)
                value = quotesMap
            }
        }
    }
