...
internal class ChannelStateLogic(
    ...
    private fun normalizeReplyMessages(quotedMessage: Message): List<Message>? {
        return getAllReplies(quotedMessage)?.map { replyMessage ->
            replyMessage.copy(
                // TODO tuyendv sửa text tin nhắn deleted để update giao diện message reply realtime, vì chưa rõ cơ chế thay đổi UI
                replyTo = quotedMessage.copy(text = if (quotedMessage.isDeleted()) quotedMessage.text + " " else quotedMessage.text),
