...
import network.ermis.client.sync.stringify
...
import network.ermis.core.errors.isStatusBadRequest
import network.ermis.core.errors.isValidationError
import network.ermis.core.internal.coroutines.Tube
...
internal class SyncManager(
    ...
    private suspend fun retryReactionDeletion(
        ...
        return if (reaction.deletedAt.exceedsSyncThreshold()) {
            ...
            val cid = "${reaction.channelType}:${reaction.channelId}"
            chatClient.deleteReaction(reaction.messageId, reaction.type, cid = cid)
        }
    }
    ...
    private suspend fun retryDeletionOfMessageWithSyncedAttachments(
        ...
        return if (message.deletedAt.exceedsSyncThreshold()) {
            ...
            channelClient.deleteMessage(message.id, channelClient.channelId, channelClient.channelType).await()
        }
    }
