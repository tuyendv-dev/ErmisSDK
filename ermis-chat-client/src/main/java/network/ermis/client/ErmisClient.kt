...
import network.ermis.client.api.ErrorCall
import network.ermis.client.api.SSEClient
...
import network.ermis.client.api.models.identifier.AddDeviceIdentifier
import network.ermis.client.api.models.identifier.DeleteDeviceIdentifier
import network.ermis.client.api.models.identifier.DeleteMessageIdentifier
import network.ermis.client.api.models.identifier.DeleteReactionIdentifier
import network.ermis.client.api.models.identifier.GetDevicesIdentifier
import network.ermis.client.api.models.identifier.GetMessageIdentifier
import network.ermis.client.api.models.identifier.GetRepliesIdentifier
import network.ermis.client.api.models.identifier.GetRepliesMoreIdentifier
import network.ermis.client.api.models.identifier.HideChannelIdentifier
import network.ermis.client.api.models.identifier.MarkAllReadIdentifier
import network.ermis.client.api.models.identifier.MarkReadIdentifier
import network.ermis.client.api.models.identifier.QueryChannelIdentifier
import network.ermis.client.api.models.identifier.QueryChannelsIdentifier
import network.ermis.client.api.models.identifier.QueryMembersIdentifier
import network.ermis.client.api.models.identifier.SendEventIdentifier
import network.ermis.client.api.models.identifier.SendGiphyIdentifier
import network.ermis.client.api.models.identifier.SendMessageIdentifier
import network.ermis.client.api.models.identifier.SendReactionIdentifier
import network.ermis.client.api.models.identifier.ShuffleGiphyIdentifier
import network.ermis.client.api.models.identifier.UpdateMessageIdentifier
import network.ermis.client.attachment.AttachmentsSender
import network.ermis.client.audio.AudioPlayer
import network.ermis.client.audio.NativeMediaPlayerImpl
import network.ermis.client.audio.StreamMediaPlayer
...
import network.ermis.client.clientstate.UserState
...
import network.ermis.client.debugger.StubChatClientDebugger
import network.ermis.client.di.ChatModule
import network.ermis.client.errorhandler.ErrorHandler
import network.ermis.client.errorhandler.onCreateChannelError
import network.ermis.client.errorhandler.onMessageError
import network.ermis.client.errorhandler.onQueryMembersError
import network.ermis.client.errorhandler.onReactionError
...
import network.ermis.client.events.HasOwnUser
...
import network.ermis.client.events.UserEvent
import network.ermis.client.events.UserUpdatedEvent
import network.ermis.client.header.VersionPrefixHeader
import network.ermis.client.helpers.AppSettingManager
import network.ermis.client.helpers.CallPostponeHelper
import network.ermis.client.interceptor.message.internal.PrepareMessageLogicImpl
import network.ermis.client.logger.ChatLogLevel
import network.ermis.client.logger.ChatLoggerConfigImpl
import network.ermis.client.logger.ChatLoggerHandler
import network.ermis.client.logger.StreamLogLevelValidator
import network.ermis.client.logger.StreamLoggerHandler
import network.ermis.client.notifications.ChatNotifications
import network.ermis.client.notifications.PushNotificationReceivedListener
...
import network.ermis.client.parser.adapters.CustomObjectDtoAdapter
...
import network.ermis.client.persistance.noop.NoOpRepositoryFactory
...
import network.ermis.client.token.CacheableTokenProvider
import network.ermis.client.token.ConstantTokenProvider
...
import network.ermis.client.user.storage.SharedPreferencesCredentialStorage
import network.ermis.client.user.storage.UserCredentialStorage
import network.ermis.client.utils.ProgressCallback
import network.ermis.client.utils.TokenUtils
import network.ermis.client.utils.extensions.ATTACHMENT_TYPE_FILE
import network.ermis.client.utils.extensions.ATTACHMENT_TYPE_IMAGE
import network.ermis.client.utils.extensions.cidToTypeAndId
import network.ermis.client.utils.extensions.internal.isLaterThanDays
import network.ermis.client.utils.internal.toggle.ToggleService
import network.ermis.client.utils.mergePartially
import network.ermis.client.utils.observable.ChatEventsObservable
import network.ermis.client.utils.observable.Disposable
import network.ermis.client.utils.retry.NoRetryPolicy
...
/**
 * The ErmisClient is the main entry point for all low-level operations on chat
 */
@Suppress("NEWER_VERSION_IN_SINCE_KOTLIN", "TooManyFunctions", "LargeClass")
public class ErmisClient
@Suppress("LongParameterList")
internal constructor(
    public val config: ErmisClientConfig,
    ...
    public fun inheritScope(block: (Job) -> CoroutineContext): CoroutineScope {
        if (userScope.userId.value == null) {
            ...
            clientDebugger.onNonFatalErrorOccurred(
                tag = TAG,
                src = "inheritScope",
                desc = "ErmisClient::connectUser() must be called before inheriting scope",
    ...
    internal inline fun <reified F : DependencyResolver, reified T : Any> resolveFactoryDependency(): T {
        ...
        val resolver = pluginFactories.find { plugin ->
            ...
            "Factory '${F::class.qualifiedName}' was not found. Did you init it within ErmisClient?",
        )
    ...
    internal inline fun <reified P : DependencyResolver, reified T : Any> resolvePluginDependency(): T {
        ...
        if (initState != InitializationState.COMPLETE) {
            ...
            throw IllegalStateException("ErmisClient::connectUser() must be called before resolving any dependency")
        }
        val resolver = plugins.find { plugin ->
            plugin is P
        } ?: throw IllegalStateException(
            "Plugin '${P::class.qualifiedName}' was not found. Did you init it within ErmisClient?",
        )
        return resolver.resolveDependency(T::class)
            ?: throw IllegalStateException(
    ...
    /**
     * Filter channels by projectId API calls.
     */
    private var projectId: String? = null
    private var projectName: String? = null
    public fun setProjectId(newProjectId: String?, projectName: String?) {
        this.projectId = newProjectId
        this.projectName = projectName
        api.setProjectId(newProjectId)
    }
    public fun clearIdInfo() {
        this.projectId = null
        this.projectName = null
    }
    public fun getProjectId() : String? {
        return this.projectId
    }
    public fun getProjectName() : String? {
        return this.projectName
    }
    public fun isInErmisProject(): Boolean {
        return projectId == ERMIS_PROJECT_ID
    }
    ...
    /**
     * Initializes [ErmisClient] for a specific user. The [tokenProvider] implementation is used
    ...
    @CheckResult
    public fun userRegister(userId: String, userName: String, password: String): Call<UserRegisterResponse> =
        api.userRegister(userId, userName, password)

    @CheckResult
    public fun userLogin(userId: String, password: String): Call<UserLoginResponse> =
        api.userLogin(userId, password)

    /**
     * Initializes [ErmisClient] for a specific user.
    ...
    /**
     * Initializes [ErmisClient] for a specific user using the given user [token].
     * Check [ErmisClient.connectUser] with [TokenProvider] parameter for advanced use cases.
     *
    ...
    /**
     * Initializes [ErmisClient] with stored user data.
     * Caution: This method doesn't establish connection to the web socket, you should use [connectUser] instead.
     *
     * This method initializes [ErmisClient] to allow the use of Stream REST API client.
     ...
     */
    @CheckResult
    public fun deleteReaction(messageId: String, reactionType: String, cid: String): Call<Message> {
        val currentUser = getCurrentUser()
        val (channelType, channelId) = cid.cidToTypeAndId()
        return api.deleteReaction(messageId = messageId, reactionType = reactionType, channelType = channelType, channelId = channelId)
        ...
        CoroutineCall(clientScope) {
            ...
            when (isUserSet()) {
                ...
                false -> {
                    ...
                    Result.Failure(
                        Error.GenericError(
                            message = "ErmisClient can't be disconnected because user wasn't connected previously",
                        ),
                    )
                }
            }
    ...
    public fun deleteDevice(device: Device): Call<Unit> {
        return api.deleteDevice(DeleteDeviceRequest(device.token))
     ...
     * Be sure to initialize ErmisClient before calling this method!
     *
    ...
    public fun deleteMessage(messageId: String, channelId: String, channelType: String, hard: Boolean = false): Call<Message> {
        ...
        return api.deleteMessage(messageId, channelId, channelType, hard)
            .doOnStart(userScope) {
                plugins.forEach { listener ->
    ...
    public fun queryChannelsInternal(request: QueryChannelsRequest): Call<List<Channel>> {
        logger.d { "[queryChannelsInternal] vao day: ${request.offset}, limit: ${request.limit}" }
    ...
    public fun queryChannels(request: QueryChannelsRequest): Call<List<Channel>> {
        ...
        return queryChannelsInternal(request = request).doOnStart(userScope) {
            ...
            plugins.forEach { listener ->
                logger.v { "[queryChannels] plugin: ${listener::class.qualifiedName} #doOnResult;$result" }
    ...
    @CheckResult
    public fun banMembersChannel(channelType: String, channelId: String, userIds: List<String>): Call<Channel> {
        return api.banMembersChannel(channelType, channelId, userIds)
    }

    @CheckResult
    public fun unbanMembersChannel(channelType: String, channelId: String, userIds: List<String>): Call<Channel> {
        return api.unbanMembersChannel(channelType, channelId, userIds)
    }

    @CheckResult
    public fun demoteMembersChannel(channelType: String, channelId: String, userIds: List<String>): Call<Channel> {
        return api.demoteMembersChannel(channelType, channelId, userIds)
    }

    @CheckResult
    public fun promoteMembersChannel(channelType: String, channelId: String, userIds: List<String>): Call<Channel> {
        return api.promoteMembersChannel(channelType, channelId, userIds)
    }

    @CheckResult
    public fun updatePermisionMembersChannel(
        channelType: String,
        channelId: String,
        add: List<String>,
        delete: List<String>
    ): Call<Channel> {
        return api.updatePermisionMembersChannel(channelType, channelId, add, delete)
    }

    @CheckResult
    public fun getAttachmentsOfChannel(
        channelType: String,
        channelId: String,
    ): Call<AttachmentResponse> {
        return api.getAttachmentsOfChannel(channelType, channelId)
    }
    ...
    public fun searchMessageOfChannel(
        ...
        return api.searchMessageOfChannel(cid, searchTerm, limit, offset)
    }
    ...
    @CheckResult
    public fun rejectInvite(channelType: String, channelId: String): Call<EmptyResponse> {
    ...
    @CheckResult
    public fun getUserInfo(id: String): Call<User> {
        return api.getUserInfo(id, projectId)
            // .doOnResult(userScope) { result ->
            //     plugins.forEach { plugin ->
            //         logger.v { "[onGetInfoCurrentUserResult] getUserInfo #doOnResult; plugin: ${plugin::class.qualifiedName}" }
            //         plugin.onGetInfoCurrentUserResult(result)
            //     }
            // }
    }

    @CheckResult
    public fun getUserInfoNoAuth(id: String, projectId: String): Call<User> {
        return api.getUserInfoNoAuth(id, projectId)
    }

    @CheckResult
    public fun updateUserProfile(name: String? = null, phone: String? = null, about_me: String? = null): Call<User> {
        return api.updateUserProfile(UpdateUserProfileRequest(name = name, phone = phone, about_me = about_me))
    }

    @CheckResult
    public fun updateUserAvatar(file: File): Call<String> {
        return api.updateUserAvatar(file)
    }
     ...
     */
    @CheckResult
    public fun queryUsers(
        querySearch: String,
        page: Int = 0,
        usersLimit: Int = 1000
    ): Call<List<User>> {
        val query = QueryUsersRequest(name = querySearch, page = page, limit = usersLimit)
        return api.queryUsers(query, projectId)
            .doOnResult(userScope) { result ->
                plugins.forEach { plugin ->
                    logger.v { "[queryUsers] #doOnResult; plugin: ${plugin::class.qualifiedName}" }
                    plugin.onQueryUserListResult(
                        result
                    )
                    logger.v { "[queryUsers] result: ${result.stringify { "Users count=${it.size}" }}" }
                }
            }
    }

    public fun getUsersByIds(userIds: List<String>): Call<List<User>> {
        return api.getUsersByIds(UsersByIdsRequest(users = userIds, project_id = projectId))
            .doOnResult(userScope) { result ->
                plugins.forEach { plugin ->
                    logger.v { "[getUsersByIds] #doOnResult; plugin: ${plugin::class.qualifiedName}" }
                    plugin.onQueryUserListResult(
                        result
                    )
                    logger.v { "[getUsersByIds] result: ${result.stringify { "Users count=${it.size}" }}" }
                }
            }
    }

    public fun getUsersChange() {
        SSEClient(tokenManager.getToken()) { userChange ->
            plugins.forEach { plugin ->
                userScope.launch {
                    plugin.onQueryUserListResult(
                        Result.Success(listOf(userChange))
                    )
                }
            }
        }
    ...
    public fun getListContactIds(): Call<List<String>> {
        return api.getListContactIdsFromUser()
    }
    ...
    @CheckResult
    public fun sendVideoCallSignal(cid: String, action: String, type: String = "", sdp: String = ""): Call<SignalCallResponse> {
        return api.sendVideoCallSignal(cid, action, type, sdp)
    }
    ...
    @CheckResult
    public fun walletConnect(address: String): Call<WalletConnectResponse> {
        return api.walletConnect(address)
    }

    @CheckResult
    public fun walletSignin(address: String, signature: String, nonce: String): Call<GetTokenLoginResponse> {
        return api.walletSignin(address, signature, config.apiKey, nonce)
    }

    @CheckResult
    public fun getChains(): Call<ErmisChainResponse> {
        return api.getChains()
    }

    @CheckResult
    public fun getClientsByChainId(chainId: String): Call<List<ErmisClientModel>> {
        return api.getClientsByChainId(chainId)
    }

    @CheckResult
    public fun getProjects(clientId: String, chainId: String): Call<List<ErmisProject>> {
        return api.getProjects(clientId, chainId)
    }

    @CheckResult
    public fun joinNewProject(newProjectId: String): Call<ErmisChainResponse> {
        return api.joinProject(JoinProjectRequest(newProjectId))
    }

    @CheckResult
    public fun queryAllChannelOfAllProject(): Call<List<Channel>> {
        return api.queryAllChannelOfAllProject()
    }
    ...
    public fun tokenIsExpired(token: String): Boolean = tokenUtils.getTokenExpired(token)
    ...
    /**
     * Builder to initialize the singleton [ErmisClient] instance and configure its parameters.
     *
    ...
    public class Builder(private val apiKey: String, private val appContext: Context) : ErmisClientBuilder() {

        private var baseUrl: String = ConfigBuilder.URL_BASE_CHAT
        ...
        /**
         * By default, ErmisClient performs a dummy HTTP call to the Stream API
        ...
        public override fun build(): ErmisClient {
            return super.build()
        }

        @SuppressWarnings("LongMethod")
        override fun internalBuild(): ErmisClient {
            if (apiKey.isEmpty()) {
            ...
            instance?.run {
                Log.e(
                    "Chat",
                    "[ERROR] You have just re-initialized ErmisClient, old configuration has been overridden [ERROR]",
                )
            }

            // Use clear text traffic for instrumented tests
            // val isLocalHost = true//baseUrl.contains("localhost")
            ...
            val config = ErmisClientConfig(
                apiKey = apiKey,
                httpUrl = "$httpProtocol://$baseUrl/",
                cdnHttpUrl = "$httpProtocol://$cdnUrl/",
                walletHttpUrl = ConfigBuilder.URL_AUTH_CHAT,
    ...
    public abstract class ErmisClientBuilder public constructor() {
        /**
        ...
        /**
         * Create a [ErmisClient] instance based on the current configuration
         * of the [Builder].
         */
        public open fun build(): ErmisClient = internalBuild()
            .also {
                instance = it
            }

        public abstract fun internalBuild(): ErmisClient
    }
    ...
    public companion object {
        ...
        public const val ERMIS_CHAIN_ID: Int = 0
        public const val ERMIS_PROJECT_ID: String = "b44937e4-c0d4-4a73-847c-3730a923ce83"
        public const val ERMIS_PROJECT_NAME: String = "ERMIS_MVP"
        ...
        private var instance: ErmisClient? = null
        ...
        @JvmStatic
        public fun instance(): ErmisClient {
            return instance
                ?: throw IllegalStateException(
                    "ErmisClient.Builder::build() must be called before obtaining ErmisClient instance",
                )
        }
         ...
         * @throws IllegalStateException if called before initializing ErmisClient
         */
        @Throws(IllegalStateException::class)
        @JvmStatic
        ...
        @Throws(IllegalStateException::class)
        private fun ensureClientInitialized(): ErmisClient {
            check(isInitialized) { "ErmisClient should be initialized first!" }
            return instance()
        }
