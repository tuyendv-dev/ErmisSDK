...
import network.ermis.state.extensions.setMessageForReply
...
import network.ermis.ui.utils.extensions.toMessageListItemWrapper
...
public class MessageListViewModel(
     ...
     */
    public val logger: TaggedLogger by taggedLogger("Chat:MessageListVM")
    ...
    public val read: LiveData<ChannelUserRead?> =
        channelState.flatMapLatest { it!!.read }.asLiveData()
    ...
    public fun onEvent(event: Event) {
        ...
        when (event) {
            ...
            is Event.MessageReaction -> {
                onMessageReaction(event.message, event.reactionType, event.channelType, event.channelId)
            }
            is Event.MuteUser -> {
     ...
     */
    private fun onMessageReaction(message: Message, reactionType: String, channelType: String, channelId: String) {
