...
import network.ermis.client.utils.message.isThreadReply
...
import network.ermis.ui.common.state.messages.CustomAction
...
import network.ermis.ui.common.state.messages.React
...
import network.ermis.ui.common.state.messages.list.GiphyAction
...
import network.ermis.ui.components.databinding.MessageListViewBinding
...
import network.ermis.ui.view.messages.dialog.ModeratedMessageDialogFragment
import network.ermis.ui.view.MessageListView.AttachmentDownloadHandler
import network.ermis.ui.view.MessageListView.BottomEndRegionReachedHandler
import network.ermis.ui.view.MessageListView.ConfirmDeleteMessageHandler
import network.ermis.ui.view.MessageListView.ConfirmFlagMessageHandler
import network.ermis.ui.view.MessageListView.CustomActionHandler
import network.ermis.ui.view.MessageListView.EndRegionReachedHandler
import network.ermis.ui.view.MessageListView.ErrorEventHandler
import network.ermis.ui.view.MessageListView.FlagMessageResultHandler
import network.ermis.ui.view.MessageListView.GiphySendHandler
import network.ermis.ui.view.MessageListView.LastMessageReadHandler
import network.ermis.ui.view.MessageListView.MessageDeleteHandler
import network.ermis.ui.view.MessageListView.MessageEditHandler
import network.ermis.ui.view.MessageListView.MessageFlagHandler
import network.ermis.ui.view.MessageListView.MessageListItemTransformer
import network.ermis.ui.view.MessageListView.MessagePinHandler
import network.ermis.ui.view.MessageListView.MessageReactionHandler
import network.ermis.ui.view.MessageListView.MessageReplyHandler
import network.ermis.ui.view.MessageListView.MessageRetryHandler
import network.ermis.ui.view.MessageListView.MessageUnpinHandler
import network.ermis.ui.view.MessageListView.ModeratedMessageOptionHandler
import network.ermis.ui.view.MessageListView.ThreadStartHandler
import network.ermis.ui.view.messages.adapter.MessageListItem
import network.ermis.ui.view.messages.adapter.MessageListItemViewHolderFactory
import network.ermis.ui.view.messages.adapter.MessageListListenerContainerImpl
...
import network.ermis.ui.view.messages.background.MessageBackgroundFactoryImpl
import network.ermis.ui.view.messages.internal.HiddenMessageListItemPredicate
import network.ermis.ui.view.messages.internal.MessageListScrollHelper
import network.ermis.ui.view.messages.options.MessageOptionItem
import network.ermis.ui.view.messages.options.MessageOptionItemsFactory
import network.ermis.ui.view.messages.options.MessageOptionsDialogFragment
import network.ermis.ui.font.setTextStyle
import network.ermis.ui.model.MessageListItemWrapper
import network.ermis.ui.navigation.destinations.AttachmentDestination
import network.ermis.ui.navigation.destinations.WebLinkDestination
import network.ermis.ui.utils.ListenerDelegate
import network.ermis.ui.utils.extensions.activity
import network.ermis.ui.utils.extensions.copyToClipboard
...
import network.ermis.ui.view.messages.DefaultShowAvatarPredicate
import network.ermis.ui.view.messages.EndlessMessageListScrollListener
import network.ermis.ui.view.messages.MessageListViewStyle
import java.util.Locale
import network.ermis.ui.common.state.messages.Flag as FlagAction
...
public class MessageListView : ConstraintLayout {
    ...
    private lateinit var binding: MessageListViewBinding
    ...
    private var onScrollToUnreadMessages = OnScrollToUnreadMessages {
        throw IllegalStateException("onScrollToUnreadMessages must be set")
    }


    private var confirmDeleteMessageHandler = ConfirmDeleteMessageHandler { _, confirmCallback ->
        AlertDialog.Builder(context)
            .setTitle(R.string.ermis_ui_message_list_delete_confirmation_title)
            .setMessage(R.string.ermis_ui_message_list_delete_confirmation_message)
            .setPositiveButton(R.string.ermis_ui_message_list_delete_confirmation_positive_button) { dialog, _ ->
                dialog.dismiss()
                confirmCallback()
            }
            .setNegativeButton(R.string.ermis_ui_message_list_delete_confirmation_negative_button) { dialog, _ ->
                dialog.dismiss()
            }
            .show()
    }
    ...
    private var confirmFlagMessageHandler = ConfirmFlagMessageHandler { _, confirmCallback ->
        AlertDialog.Builder(context)
            .setTitle(R.string.ermis_ui_message_list_flag_confirmation_title)
            .setMessage(R.string.ermis_ui_message_list_flag_confirmation_message)
            .setPositiveButton(R.string.ermis_ui_message_list_flag_confirmation_positive_button) { dialog, _ ->
                dialog.dismiss()
                confirmCallback()
            }
            .setNegativeButton(R.string.ermis_ui_message_list_flag_confirmation_negative_button) { dialog, _ ->
                dialog.dismiss()
            }
            .show()
    }
    ...
    private var errorEventHandler = ErrorEventHandler { error ->
        when (error) {
            is MessageListController.ErrorEvent.MuteUserError -> R.string.ermis_ui_message_list_error_mute_user
            is MessageListController.ErrorEvent.UnmuteUserError -> R.string.ermis_ui_message_list_error_unmute_user
            is MessageListController.ErrorEvent.BlockUserError -> R.string.ermis_ui_message_list_error_block_user
            is MessageListController.ErrorEvent.FlagMessageError -> R.string.ermis_ui_message_list_error_flag_message
            is MessageListController.ErrorEvent.PinMessageError -> R.string.ermis_ui_message_list_error_pin_message
            is MessageListController.ErrorEvent.UnpinMessageError -> R.string.ermis_ui_message_list_error_unpin_message
            is MessageListController.ErrorEvent.MarkUnreadError ->
                R.string.ermis_ui_message_list_error_mark_as_unread_message
        }.let(::showToast)
    }
    ...
    private var haveSetUnreadMessages: Boolean = false
    private val mListMessageDisplayIds: ArrayList<String> = arrayListOf()
    ...
    private val defaultAttachmentDownloadClickListener =
        OnAttachmentDownloadClickListener { attachment ->
            attachmentDownloadHandler.onAttachmentDownload {
                Toast.makeText(
                    context,
                    context.getString(R.string.ermis_ui_message_list_download_started),
                    Toast.LENGTH_SHORT,
                ).show()
                ErmisClient.instance().downloadAttachment(context, attachment)
            }
    ...
    private fun init(attr: AttributeSet?) {
        binding = MessageListViewBinding.inflate(streamThemeInflater, this)
        ...
        layoutTransition = LayoutTransition().also {
            // TODO TuyenDv fix java.lang.IllegalArgumentException: Tmp detached view should be removed from RecyclerView before it can be recycled
    ...
    private fun initRecyclerView() {
        ...
        binding.llGoToUnreadMessages.setOnClickListener {
            if (lastReadMessageId != null) {
                onScrollToUnreadMessages.onScrollToUnreadMessages(lastReadMessageId!!)
                binding.llGoToUnreadMessages.isVisible = false
            }
        }
        binding.ivCloseUnreadMessages.setOnClickListener {
            binding.llGoToUnreadMessages.isVisible = false
        }
    }
    ...
    private fun configureAttributes(attributeSet: AttributeSet?) {
        context.obtainStyledAttributes(
            ...
            tArray.getInteger(
                R.styleable.MessageListView_ermisUiLoadMoreThreshold,
                LOAD_MORE_THRESHOLD,
    ...
    public fun showTypingStateLabel(typingUsers: List<User>) {
        // TODO tuyendv filter member typing get name show typing
        val allMembers = channel.members.map { it.user }
        val typingMembers = ArrayList<User> ()
        typingUsers.forEach { user ->  
            val member = allMembers.first { it.id == user.id }
            typingMembers.add(member)
        }
        if (typingMembers.isEmpty()) {
            binding.typingIndicatorView.isVisible = false
        } else {
            binding.typingIndicatorView.isVisible = true
            binding.typingIndicatorView.setTypingUsers(typingMembers)
        }
    }
    ...
    public fun setChannelUserRead(unreadMessages: Int, lastReadMessageId: String) {
        this.lastReadMessageId = lastReadMessageId
        this.haveSetUnreadMessages = true
        if (unreadMessages > 0 && mListMessageDisplayIds.contains(lastReadMessageId).not()) {
            binding.llGoToUnreadMessages.isVisible = true
            binding.unreadCountTextView.text = unreadMessages.toString()
            mListMessageDisplayIds.clear()
        }
    }
    ...
    private fun initAdapter() {
        ...
        adapter.messageDisplayListener = { messageDisplayId ->
            if (!haveSetUnreadMessages) {
                mListMessageDisplayIds.add(messageDisplayId)
            }
            if (messageDisplayId == this.lastReadMessageId) {
                binding.llGoToUnreadMessages.isVisible = false
            }
        }
    }
    ...
    public fun setOnScrollToUnreadMessages(onScrollToUnreadMessages: OnScrollToUnreadMessages) {
        this.onScrollToUnreadMessages = onScrollToUnreadMessages
    }
    ...
    public fun interface OnScrollToUnreadMessages {
        public fun onScrollToUnreadMessages(unreadMessagesId: String)
    }
    //endregion
