...
import network.ermis.client.utils.extensions.isAnonymousChannel
...
import network.ermis.ui.components.databinding.ChannelListItemBackgroundViewBinding
import network.ermis.ui.components.databinding.ChannelListItemForegroundViewBinding
import network.ermis.ui.components.databinding.ChannelListItemViewBinding
...
import network.ermis.ui.view.channels.adapter.viewholder.SwipeViewHolder
...
import network.ermis.ui.utils.extensions.readCount
...
internal class ChannelViewHolder @JvmOverloads constructor(
    ...
    private val binding: ChannelListItemViewBinding = ChannelListItemViewBinding.inflate(
    ...
    private val menuItemWidth = context.getDimension(R.dimen.ermis_ui_channel_list_item_option_icon_width).toFloat()
        ...
        binding.itemForegroundView.apply {
            diff.run {
                ...
                if (lastMessageChanged || typingUsersChanged) {
                    ...
                    val allMembers = channelItem.channel.members.map { it.user }
                    val typingMembers = ArrayList<User> ()
                    channelItem.typingUsers.forEach { user ->
                        val member = allMembers.first { it.id == user.id }
                        typingMembers.add(member)
                    }
                    typingIndicatorView.setTypingUsers(typingMembers)
                }
    ...
    private fun ChannelListItemForegroundViewBinding.configureChannelNameLabel(
        lastMessage: Message?,
    ...
    private fun ChannelListItemForegroundViewBinding.configureAvatarView() {
        channelAvatarView.setChannel(channel)
    }

    private fun ChannelListItemForegroundViewBinding.configureLastMessageLabelAndTimestamp(
        lastMessage: Message?,
    ) {
    ...
    private fun ChannelListItemForegroundViewBinding.configureUnreadCountBadge() {
    ...
    private fun ChannelListItemForegroundViewBinding.configureCurrentUserLastMessageStatus(
        lastMessage: Message?,
    ) {
    ...
    private fun ChannelListItemBackgroundViewBinding.applyStyle(style: ChannelListViewStyle) {
    ...
    private fun ChannelListItemForegroundViewBinding.applyStyle(style: ChannelListViewStyle) {
