...
import network.ermis.ui.components.databinding.ActivityAttachmentGalleryBinding
import network.ermis.ui.view.gallery.internal.AttachmentGalleryPagerAdapter
...
import network.ermis.ui.view.gallery.options.AttachmentGalleryOptionsDialogFragment
import network.ermis.ui.view.gallery.overview.MediaAttachmentDialogFragment
...
public class AttachmentGalleryActivity : AppCompatActivity() {
    ...
    private lateinit var binding: ActivityAttachmentGalleryBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityAttachmentGalleryBinding.inflate(streamThemeInflater)
    ...
    private fun shareImage(attachment: Attachment) {
        ...
        if (imageUrl != null) {
            ...
            shareMediaJob = lifecycleScope.launch(DispatcherProvider.Main) {
                ErmisImageLoader.instance().loadAsBitmap(
    ...
    private fun launchShareActivity(
        ...
        ContextCompat.startActivity(
            ...
            Intent.createChooser(
                ...
                getString(R.string.ermis_ui_attachment_gallery_share),
            ),
            null,
        )
    }
    ...
    private fun toastFailedShare() {
        Toast.makeText(
            applicationContext,
            applicationContext.getString(R.string.ermis_ui_attachment_gallery_share_error),
            Toast.LENGTH_SHORT,
        ).show()
    }
    ...
    private fun onGalleryPageSelected(position: Int) {
        binding.mediaInformationTextView.text = getString(
            R.string.ermis_ui_attachment_gallery_count,
            position + 1,
    ...
    private fun getRelativeAttachmentDate(createdAt: Date): String {
        ...
        return getString(
            R.string.ermis_ui_attachment_gallery_date,
            relativeDay,
    ...
    private fun enterFullScreenMode() {
        with(binding) {
            ...
            galleryViewPager.setBackgroundColor(getColorCompat(R.color.ui_literal_black))
            ConstraintSet().apply {
    ...
    private fun exitFullScreenMode() {
        with(binding) {
            ...
            galleryViewPager.setBackgroundColor(getColorCompat(R.color.ui_white_snow))
            ConstraintSet().apply {
    ...
    private fun setSharingInProgressUi() {
        with(binding) {
            mediaInformationTextView.text =
                applicationContext.getString(R.string.ermis_ui_attachment_gallery_preview_preparing)
            progressBar.visibility = View.VISIBLE

            val drawable =
                ContextCompat.getDrawable(applicationContext, R.drawable.ic_clear)?.mutate()
                    ?.apply { setTint(ContextCompat.getColor(applicationContext, R.color.ui_black)) }
